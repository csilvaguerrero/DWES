	...
	<body>
		<div id=divFormulario>
			<form id=formulario method="post" action="index.php?controlador=Retos&metodo=modificarReto&id=<?php echo $datos["dato"][1]["id"];?>">
                ...
                <br>
                <label for="selectFormulario" class="labelFecha">Profesor
                    <br><br>
                    <select class="selectFormulario" name="idProfesor">
                    <?php
                            while($mostrarProfesores = $datos["dato"][3]->fetch_array(MYSQLI_ASSOC)){		
                                if ($datos["dato"][1]["idProfesor"] == $mostrarProfesores["id"]){?>
                                   <option selected="true" value=<?php echo $mostrarProfesores["id"]?>><?php echo $mostrarProfesores["nombre"]; ?></option>
                                   <?php }else{?>
                         
							<option value=<?php echo $mostrarProfesores["id"]?>><?php echo $mostrarProfesores["nombre"]; ?></option>
						<?php }}?>
                    </select>
                </label>
                <br>
